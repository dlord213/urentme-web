  ...
  return (
    <div className="animate-in fade-in duration-300 space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatsCard
          title="Revenue (MTD)"
          value={`₱${stats.revenueMtd.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
          icon={PhilippinePeso}
          color="accent"
        />
      </div>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="font-bold text-primary">
                      {calculatePercentage(occupancy.occupied, occupancy.total)}
                      %
                    </span>
                  </div>
                  <progress
              ...
              <div className="space-y-2">
                {alerts.overdueRents > 0 && (
                  <div className="flex items-center gap-3 p-2 rounded-lg bg-error/10">
                    ...
                    <span className="text-sm font-medium text-error">
                      {alerts.overdueRents} overdue rent
                      {alerts.overdueRents > 1 ? "s" : ""}
                    </span>
                  </div>
                )}
                <div className="flex items-center gap-3 p-2 rounded-lg bg-warning/10">
                  ...
                  <span className="text-sm font-medium text-warning">
                    {alerts.leaseRenewalsDue} lease renewal
                    {alerts.leaseRenewalsDue !== 1 ? "s" : ""} due (30d)
                  </span>
                </div>
                <div className="flex items-center gap-3 p-2 rounded-lg bg-info/10">
                  <TrendingUp className="w-4 h-4 text-info shrink-0" />
                  <span className="text-sm font-medium text-info">
                    {alerts.newApplications} draft lease
                    {alerts.newApplications !== 1 ? "s" : ""}
                  </span>
                </div>
              </div>
            </div>
